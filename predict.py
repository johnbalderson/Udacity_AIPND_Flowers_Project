...
from image_utility import process_image
...
#  Argumwnts:
#
#  image_dir      - path to image in directory where images are stored (default in flowers directory)
#  model_input    - location of path of model checkpoint (default is checkpoint.pth)
#  top_k          - set number of top k most likely classes (default is 5)
#  category_names - set file for mapping of flower categories to category names (default is cat_to_name.json)
#  device         - set device mode cuda or cpu (defailt is cuda)
...
parser.add_argument('--image_dir', type=str, default='./flowers/test/44/image_01516.jpg',
                        help='set path of the flower image (default=./flowers/test/44/image_01516.jpg)')
...
# check to see if cuda is available otherwise use cpu
...
# then returns the most likely image class index and its associated probability
